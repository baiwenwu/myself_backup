							...
							)
{
	...
	memset(node->zipBuff,0,byteLen*3);
	//node->zipBuff=(uchar *)malloc(byteLen*3);
...
{//20170306
	...
	int ret=gppHybirdCode(wavTree->bitBuff, wavTree->bitLen, 
		wavTree->zipBuff, HBblockSize);
	//int ret=runLengthGammaCode(wavTree->bitBuff, wavTree->bitLen,wavTree->zipBuff);
	if (ret<0)
	{
		errProcess("runLengthGammaCode",ret);
		return ret;
...
int compressWaveletTree(waveletTree root,NodeCodeType type,u32 HBblockSize)
{
	int ret;
	if (!root)
	{
	...
	case RLE_DELTA:
		...
		break;
	case HYBIRD:
		ret=compressWaveletTreeWithHybird(root,HBblockSize);
		break;
	default:
		return ERR_PARAMETER;
		break;
