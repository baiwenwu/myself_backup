...
int multiCharHuffCode(huffmanTree tree, char(*codeTable)[CODE_MAX_LEN])
{
	...
	if (tree->leftChild == NULL && tree->righChild == NULL)
	{
		...
		strcpy_s(codeTable[tree->label],255, tree->code);
		return 0;
	}
	//cout << testStr(tree->code) << endl;
	...
	strcpy_s(tree->leftChild->code, codeLen+1, tree->code);
	strcpy_s(tree->righChild->code, codeLen+1, tree->code);
